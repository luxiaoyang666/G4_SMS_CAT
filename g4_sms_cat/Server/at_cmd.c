...
int send_at_cmd(ComportAttr *comport,char *atcmd,char *expected_recv,char *rmsg,int msgsize,int timeout)
{
    ...
    if(comport_recv(comport,temp_msg,msgsize,timeout) <= 0)
    {
...
int check_at_ok(ComportAttr *comport)
{         
    int    retval;
...
int check_sim_ready(ComportAttr *comport)
{           
    int    retval;
 ...
 * 返回值：成功返回0，失败返回-1
 */
int check_sim_operator(ComportAttr *comport)
{
...
int check_sim_allready(ComportAttr *comport)
{
    ...
    {
		if(check_sim_registe(comport) < 0)
        	return -4;
	...
	{
		if(check_sim_operator(comport) < 0)
			return -5;
